...
def predict_water_issue(current_user):
    ...
    try:
        ...
        # Predict risk score and send SMS
        risk_score = None
        if predictive_model and scaler:
            X = [[float(lat), float(lng)]]
            X_scaled = scaler.transform(X)
            risk_score = predictive_model.predict_proba(X_scaled)[0, 1]
            if risk_score > 0.7 and officer_phone:
                client_twilio.messages.create(
                    body=f"High-risk {category} issue reported at {address}. Risk score: {round(risk_score, 2)}",
                    from_=twilio_phone,
                    to=officer_phone
                )
                logger.info(f"SMS sent to {officer_phone} for {address}")
        # Retrain model to incorporate new report
